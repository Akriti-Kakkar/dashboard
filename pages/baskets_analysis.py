...
class basket_analysis:
    ...
    def fund_mwr_page(self):
        st.subheader('📈 Baskets Vs S&P Analysis')
        frame = self.ch.fillna(0)
        frame = frame.drop([x for x in self.baskets_lst if x.endswith("_unmerged")],
                           axis=1)
        st.markdown("<font color='green'><b><span style='font-size: 24px;'>🔝 Top 2 Active Baskets</span></b></font>", unsafe_allow_html=True)
        ...
        st.metric(label=f"{lst_yr} PnL",
                       value=inc1,
                       delta=inc1)        
        sum_frame = frame.sum(axis=0).sort_values(ascending=False)
        ...
        sum_frame_data = sum_frame_data.style.applymap(color_kwargs, subset=["PnL"]
                                             ).set_properties(align="right", subset=["PnL"]
                                                              ).set_table_styles(
                green, axis=1).set_caption("Baskets Rankings Based On PnL")
        mwr_frame = pd.DataFrame({'Baskets': self.cap_data.columns.tolist(), 'MWR (%)': self.mwr_lst})
        ...
        mwr_frame['MWR (%)'] = mwr_frame['MWR (%)'].apply(lambda x: x*100)
        mwr_frame['MWR (%)'] = mwr_frame['MWR (%)'].astype(float)
        mwr_frame['Rank'] = mwr_frame.index+1
        mwr_frame = mwr_frame.set_index("Rank")
        #color_lst = mwr_frame["MWR (%)"].apply(lambda x: color_kwargs(x,0)).tolist()
        mwr_frame["MWR (%)"] = mwr_frame["MWR (%)"].round(2)
        mwr_frame["MWR (%)"] = mwr_frame["MWR (%)"].apply(lambda x: str(x)+"%")
        mwr_frame = mwr_frame.style.applymap(color_kwargs, subset=["MWR (%)"]
                                             ).set_properties(align="right", subset=["MWR (%)"]
                                                              ).set_table_styles(
                green, axis=1).set_caption("Baskets Rankings Based On Returns")
        ...
        if baskets_radio == 'Baskets 2024':
            ...
            for x, y, z, l, a, b, c, d, e in list(zip(self.baskets, self.init_cap, 
                ...
                locals()[f'{x}_button'] = st.sidebar.button(f'''{x} 
                                                           ...
                                                           \n    Invested Amount: {y.replace('.00','')}   Deposits: {z.replace('.00','')}
